...
public class PersistenceServiceStructureImplBase {
	...
	*/
	...
	public boolean isJpaEnabledForKradClass(Class clazz) {
		...
		boolean isModuleJpaEnebled = false;
		
		if (clazz.getPackage() != null) {
			ModuleService moduleService = KRADServiceLocatorWeb.getKualiModuleService().getResponsibleModuleService(clazz);
			
			if (moduleService != null) {
				isModuleJpaEnebled = OrmUtils.isJpaEnabled(moduleService.getModuleConfiguration().getNamespaceCode());
			}
		}
		
		return jpaAnnotated && (jpaEnabled || prefixJpaEnabled || isModuleJpaEnebled);
	}
